...
class TcpClient:
    ...
    def __init__(self, ip, port, timeout=2, needPrint=False, needfixMixPackets=False):
        ...
        self.EVENT_CONNECT_SUCCESS = pyevent.Event(
            self.__eventManager, 'EVENT_CONNECT_SUCCESS')
        self.EVENT_CONNECT_FAIL = pyevent.Event(
            self.__eventManager, 'EVENT_CONNECT_FAIL')
            ...
            try:
                ...
                if recvdata != '':
                    if self.__needfixMixPackets:
                            ...
                            for msg in recvmsg:
                                if msg != '':
                                    self.EVENT_RECEIVE_MESSAGE.emit(
                                        (self, msg))
                                    if self.__needPrint:
                                        print('Client receive : "%s"' % msg.replace(
                                            '\r', '\\r').replace('\n', '\\n')[:-2])
                        else:
                            recvmsg = recvdata.split('</a/b>')
                            if self.__badmsg != '':
                        ...
                        if self.__needPrint:
                            print('Client receive : "%s"' % recvdata.replace(
                                '\r', '\\r').replace('\n', '\\n')[:-2])
                else:
                    time.sleep(0.001)
            except Exception as e:
